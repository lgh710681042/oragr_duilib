...
{
	//create case window
	m_hWnd = CreateWindow(GetWindowClass(), _T(""), WS_OVERLAPPED, 0, 0, 0, 0, HWND_DESKTOP, NULL, m_hInstance, this);
...
{
	/*if (uMsg == Wm_User_EventBus)
	{
	CEventBus::GetInstance()->ReceiveWinEvent(m_hWnd, uMsg, wParam, lParam);
	return FALSE;
	}*/
	...
	{
	case WM_COPYDATA:
		this->ReceiveCopydata(m_hWnd, uMsg, wParam, lParam);
		break;
	}
...
void CMainWnd::ReceiveCopydata(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
